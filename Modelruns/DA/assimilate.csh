...
#=========================================================================
# Remove some restart files to save storage space
#=========================================================================
set checkdatedir = /glade/work/huoxl/scripts/

if ( (${LND_YEAR} == 2012) || (${LND_YEAR} == 2016) ) then
    if ( $LND_DATE[2]$LND_DATE[3] == 0101 ) then
      set rmfilename = ${checkdatedir}checkyeardate.txt
   else
      set rmfilename = ${checkdatedir}check1216date.txt
   endif
else
   set rmfilename = ${checkdatedir}checkyeardate.txt
endif

foreach adate ( `cat ${rmfilename}` )
   if ( ${adate} == -$LND_DATE[2]-$LND_DATE[3] ) then
      break
   else
      if ( $LND_DATE[2]$LND_DATE[3] == 0101 ) then
         set RM_YEAR  =  `echo $LND_DATE[1] - 1 | bc`
      else
         set RM_YEAR  =  ${LND_YEAR}
      endif
      if ( -e ${CASE}.clm2_0001.r.${RM_YEAR}${adate}-$LND_DATE[4].nc ) then
         ${REMOVE} *cpl*.r.${RM_YEAR}${adate}*
         ${REMOVE} *clm2*.r.${RM_YEAR}${adate}*
         ${REMOVE} *da*.rs1.${RM_YEAR}${adate}*
         ${REMOVE} *.rh*.${RM_YEAR}${adate}*
         echo "remove the previous restart file on ${RM_YEAR}${adate}"
      endif
   endif
end
...
set OBS_FILE = ${baseobsdir}/obs_seq_arctic.out.${LND_DATE_EXT}
...
if (  -e   ${OBS_FILE} ) then
   ...
   echo "No observation file $OBS_FILE is found"
   echo "Skip the assimilate at $LND_DATE_EXT"
   # Because the first observation time is set to be Jan 1st in each year, we have to change the stop options
   if ( (${LND_MONTH}${LND_DAY} == 1226) || (${LND_MONTH}${LND_DAY} == 1227) ) then
      echo "Change STOP OPTION on $LND_DATE_EXT"
      set SD_YEAR  =  `echo $LND_DATE[1] + 1 | bc`
      ${REMOVE} rpointer.*
      ...
      ./xmlchange STOP_N=1
      ./xmlchange STOP_DATE=${SD_YEAR}0101
      ./xmlchange REST_OPTION=date
      ./xmlchange REST_N=1
      echo "STOP_DATE is ${SD_YEAR}0101"

      ./xmlchange RESUBMIT_SETS_CONTINUE_RUN=FALSE
      ./xmlchange CONTINUE_RUN=FALSE
      ./xmlchange RUN_STARTDATE=${LND_YEAR}-${LND_MONTH}-${LND_DAY}
      ./xmlchange RUN_REFDATE=${LND_YEAR}-${LND_MONTH}-${LND_DAY}
      if ( ${LND_YEAR} == 2011 ) then
      ...
      while ( $inst <= $num_instances )
         ...
         sed s/curcase/${CASE}/g user_nl_clm_temp >! temp1
         sed s/INST/${inst_string}/g temp1 >! temp2
         sed s/YEAR/${LND_YEAR}/g temp2 >! temp3
         sed s/MONTH/${LND_MONTH}/g temp3 >! temp4
         sed s/DAY/${LND_DAY}/g temp4 >! user_nl_clm_${inst_string}
         @ inst ++
      end
      ${REMOVE} temp*

      set prenh = 192
      if ( ${LND_MONTH}${LND_DAY} == 1226 ) then
         set curnh = 144
      else
         set curnh = 120
      endif
      foreach FILE ( user_nl_clm_00* )
         ${COPY} ${FILE} ${FILE}_nh
         sed -e "s#${prenh}#${curnh}#" ${FILE}_nh >! ${FILE}
         ${REMOVE} ${FILE}_nh
      end
      echo "Change nh from ${prenh} to ${curnh} hours in user_nl_clm files"
   endif

   if ( ${LND_MONTH}${LND_DAY} == 11 ) then
      echo "Change STOP OPTION on $LND_DATE_EXT"
      ${REMOVE} rpointer.*
      ...
      echo "STOP_OPTION is ${stop_n} ${stop_option}"

      ./xmlchange RUN_STARTDATE=${LND_YEAR}-0${LND_MONTH}-0${LND_DAY}
      ./xmlchange RUN_REFDATE=${LND_YEAR}-0${LND_MONTH}-0${LND_DAY}
      ...
      while ( $inst <= $num_instances )
         ...
         sed s/INST/${inst_string}/g temp1 >! temp2
         sed s/YEAR/${LND_YEAR}/g temp2 >! temp3
         sed s/MONTH/0${LND_MONTH}/g temp3 >! temp4
         sed s/DAY/0${LND_DAY}/g temp4 >! user_nl_clm_${inst_string}
         @ inst ++
      end
      ${REMOVE} temp*

      set curnh = 192
      if ( (${LND_YEAR} == 2013) || (${LND_YEAR} == 2017) ) then
         set prenh = 144
      else
         set prenh = 120
      endif
   ...
   if ( ${LND_MONTH}${LND_DAY} == 19 ) then
      cd ${CASEROOT}
      ./xmlchange RESUBMIT_SETS_CONTINUE_RUN=TRUE
   endif
...
## Set the first and the last day of the summer window period
@ fdsw = 609
@ ldsw = 906

## Change the input.nml by detecting whether the date is within the summer window.
${COPY} ${CASEROOT}/input.nml.damp4 ${CASEROOT}/input.nml
echo "${fdsw} ${ldsw} ${LND_MONTH}$LND_DATE[3]"
if ( (${fdsw} <= ${LND_MONTH}$LND_DATE[3]) && (${LND_MONTH}$LND_DATE[3] <= ${ldsw}) ) then
   ${COPY} ${CASEROOT}/input.nml.damp9 ${CASEROOT}/input.nml
   if ( ${LND_YEAR} < 2015 ) then
   if ( ${LND_MONTH}$LND_DATE[3] == ${ldsw} || ${LND_MONTH}$LND_DATE[3] == 905 ) then
      ${COPY} ${CASEROOT}/input.nml.updateall ${CASEROOT}/input.nml
   endif
   endif
endif
